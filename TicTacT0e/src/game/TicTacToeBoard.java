...
public class TicTacToeBoard implements Board, Position {
    ...
    public boolean isValid(final Move move, ValidityCheckMode mode, boolean log) {
        ...
        if (!(field[move.getRow()][move.getCol()].equals(Cell.E))) {
            if (log) System.out.printf("Cell with coordinates(%d, %d) is already occupied", move.getRow(), move.getCol());
            return false;
        }
        ...
        if (ValidityCheckMode.PREPARE.equals(mode) && currentPlayerHasObtainedValidMove) {
            if (log) System.out.printf("Player already has prepared a valid move, but has not used it yet!");
            return false;
        }

        currentPlayerHasObtainedValidMove = false;
        return true;
    }
